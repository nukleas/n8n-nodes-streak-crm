...
export class FieldsService extends AbstractService<IStreakField> {
    ...
    async query(queryParams?: Record<string, string>): Promise<IStreakField[]> {
        return this.request<IStreakField[]>('GET', `/pipelines/${this.pipelineKey}/fields`, undefined, queryParams);
    }
    ...
    async update(id: string, params: Partial<IStreakField>): Promise<IStreakField> {
        return this.request<IStreakField>('POST', `/pipelines/${this.pipelineKey}/fields/${id}`, params);
    }
    ...
    async updateBoxFieldValue(
        ...
        return this.request<IStreakFieldValue>('POST', `/boxes/${boxKey}/fields/${fieldKey}`, { value });
    }
}
